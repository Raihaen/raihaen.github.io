...
We can prove that the best classifier, called the Bayes classifier, is basically choosing for each element class \\(C_i\\) such that \\(\text{max}_i P(y_0 = C_i  \mid  X=  x_0 )\\).\
...
The Bayes error rate is theoretical minimum, equall to \\(1- \text{E}(\text{max}_j P(Y=j  \mid  X))\\)
#### \\(K\\)-closest neighbors :
This is a simple but very useful classifier, we assign to observation \\(x_0\\) probability of being in class \\(j\\) based on its closest \\(K\\) neighbors's classes : \\(P(Y=j  \mid  X = x_0)=\frac{\sum_{i \in N_0} I(y_i = j)}{K}\\) [^6].\
...
We then condition on \\(X\\) (\\(P(x,y)= P(x) \, P(y \mid x)\\)), this turns our integral into :\
\\(\text{EPE}(f) = \int (\int (Y-f(x) \, P(dy \mid dx) \, P(dx)))= E_X(E_{Y \mid X}[(Y-f(x))^2]\\) \
...
We write : \\(f(x) = \text{argmin} ._c (E[(Y-c)^2]  \mid  X=x)\\)
Deriving and setting to zero to minimize it gives us : \
\\(0 = 2 E(Y-f(x) \mid  X=x) \Rightarrow f(x) = E(Y  \mid  X=x)\\). The best estimator for \\(f\\) is the conditional mean.\
...
Another interesting note is that by changing our EPE function, say for example to \\( \mid y-c \mid \\), the optimal approximation would become \\(\hat f(x) = \text{Med}(y_i ; i \in N_x)\\), the median of \\(y\\)s in that region. This approximator gives better results than the average but is less used to to **its derivations not being connected** (check the orthograph).\
...
Now proceeding like with quantitative responses, we start with \\( \text{EPE} = E[L(G, \hat G(X))]\\), and the use conditional probability to prove it suffices to optimize locally and end up with : \\( G(X) = \arg\min_g (\sum L(G_i, g) \cdot P(G_i \mid X=x))  \\).
Now if we use the zero-one loss function, this turns into \\( G(X) = \arg\min_g (\sum_{G_i \not = g} P(G_i \mid X=x)) \\)

or just a plain \\( G(X) = \text{argmax}_g (P(g \mid X=x)) \\).
...
For a function like \\(f(x) = e^{-8 \mid  \mid X \mid  \mid ^2}\\), your estimate of any point will be very biased downward (since points are on edges) and RSS will be close to 1.\
...
\\(EPE(x_0) = Var(y_0  \mid  x_0) + Bias(\hat y_0)^2 + Var_T(\hat y_0) \\) (our usual decomposition).\

Now we know :
\\(\hat y_0\\) is unbiased, and that \\(\text{Var}(y_0  \mid  x_0) = \sigma^2\\) is a systematic error outside the scope of the model, we thus get :\
...
We saw that squared error loss leads to the regression function \\(f(x) = E(Y  \mid  X= x)\\), the class of KNN methods is a direct estimate but it fails in higher dimensions because of the curse of dimensionality on one side, and the fact that if there's some structure to the data then that can be used to reduce the vol and bias of the estimates.\
...
We can modify our example, for example, to include the case where our error variable can be dependent on \\(X\\) : \\(\text{Var}(Y  \mid  X=x) = \sigma(X)\\).\
In this new model, both the mean and the variance depend on \\(X\\) (prev one was just the mean).\
\\(Pr(Y \mid X)\\) can depend on \\(X\\) in complicated ways but these aren't possible in the additive error model (\\(f(x)+ \epsilon\\)).

We can apply the same treatement to qualitative outputs, in this case the target function is the conditional density \\(P(G \mid X)\\), this is usually modeled directly by estimating \\(P\\) (in quantitative outputs we started by modeling a relationship between \\(E(Y \mid X)\\) and \\(f\\), such an example was the additive error model, only then did we start modeling \\(f\\); this approach isn't common for qualitative outputs).\
Let's consider a qualitative example : consider a binary qualitative response \\(G\\); We can model \\(p(x)\\) to be the proba of having \\(G_1\\) as a response for \\(x\\), and (naturally) \\(1-p(x)\\) to be the proba of having \\(G_2\\) as a response. If we use a 1-0 coding then \\(E(Y \mid X=x) = p(x)\\) (since it's 1*p+ 0* (1-p) ) and \\(Var(Y \mid X) = p(x)(1-p(x))\\) additionaly (so both mean and variance depend on \\(x\\) in this model).
...
- The machine learning point of view : The machine attemps to learn \\(f\\) through a teacher providing a test set of observations \\(T\\), it can also correct its estimation \\(\hat f\\) based on differences \\( \mid y- \hat f \mid \\) in \\(T\\). Teacher mothods are either ML (analogies to human reasoning) or DL (analogies on the way the brain works).
...
- [^17] The bayesian framework is basically that the selection of parameters based on the data can be separated into 3 components following Bayes' equation : \\(P(\theta  \mid  y) = \frac{P(y \mid \theta) P(\theta)}{P(Y)}\\) , this ca then separated using the log operation : \\(log(P(y \mid \theta))\\) is called the log-likelihood (it represents how well our model fits the data), \\(log P(\theta \mid  y)\\) the log posteriori (our goal). \\(P(\theta)\\) is the log prior and it represents our model of \\(f\\), based on experience / assumptions etc. \\(log P(y)\\) is the log-evidence and is a constant related to our observations.
